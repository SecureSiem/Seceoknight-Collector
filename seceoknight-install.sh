...
display_summary() {
    ...
    systemctl status seceoknight-indexer.service --no-pager -l 2>/dev/null | grep "Active:" || echo "  seceoknight-indexer: status unknown"
    systemctl status seceoknight-manager.service --no-pager -l 2>/dev/null | grep "Active:" || echo "  seceoknight-manager: status unknown"
    systemctl status seceoknight-filebeat.service --no-pager -l 2>/dev/null | grep "Active:" || echo "  seceoknight-filebeat: status unknown"
    echo ""
    echo -e "${CYAN}Server IP Address: ${server_ip}${NC}"
    echo ""
    echo -e "${CYAN}Access Information:${NC}"
    echo "  • Seceoknight Indexer: https://${server_ip}:9200"
    echo "  • Seceoknight API: https://${server_ip}:55000"
    echo ""
    echo -e "${YELLOW}${BOLD}════════════════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}${BOLD}  COPY THESE VALUES TO YOUR DASHBOARD SERVER .env FILE      ${NC}"
    echo -e "${YELLOW}${BOLD}════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}# OpenSearch Configuration${NC}"
    ...
    echo -e "${YELLOW}${BOLD}════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "${CYAN}Configuration files saved at:${NC}"
    echo "  • /var/ossec/api/configuration/dashboard.env"
    echo "  • /var/ossec/api/configuration/dashboard.json"
    echo ""
    echo -e "${YELLOW}Important:${NC}"
    echo "  1. Copy the values above to your dashboard's .env file"
    echo "  2. Ensure your dashboard server can reach:"
    echo "     - https://${server_ip}:9200 (OpenSearch)"
    echo "     - https://${server_ip}:55000 (Wazuh API)"
    echo "  3. Configure firewall to allow connections from dashboard IP"
    echo ""
